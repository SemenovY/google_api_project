"""
Тут будет описана основная логика работы приложения, а именно, логика
управления приложением через аргументы командной строки.
"""
import argparse
...
def create_spreadsheet(service, data):
    """
    Функция будет создавать документ (spreadsheet)
    и возвращать его ID. Такая функция должна принимать объект класса
    Resource, в программе он содержится в переменной service.
    В начале будет распаковываться значение аргумента, которое состоит
...
def read_values(service, spreadsheetId):
    """
    Функция построчно возвращает считанные данные в виде списка списков.
    """
    range = "A1:E30"
...
def main(args):
    """
    Опишем логику поведения приложения в зависимости от полученных
    аргументов командной строки.
    - Аргумент для функции, которая выводит список всех документов с таблицами
    — Аргумент для функции, которая удаляет все документы с таблицами.
    """
    if args.list:
