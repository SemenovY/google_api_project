...
# Снова напишите функцию авторизации.
def auth():
...
# Напишите функцию, которая получает список всех электронных таблиц.
def get_list_obj(service):
    response = service.files().list(q='mimeType="application/vnd.google-apps.spreadsheet"')
    return response.execute()

# Напишите функцию, которая удалит все таблицы, кроме последней созданной.
